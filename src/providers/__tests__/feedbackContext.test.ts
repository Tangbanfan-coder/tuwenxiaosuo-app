...
describe('近期偏好反馈上下文', () => {
  ...
  it('渲染消息与段落反馈的覆盖关系，并隐藏完整目标段落原文', async () => {
    ...
    expect(context).not.toContain('近期偏好反馈')
    expect(context).not.toContain('首句短预览')
    expect(context).not.toContain('短指纹')
    expect(context).not.toContain('消息级（仅适用于同一消息中未单独标注的其他段落）')
    expect(context).not.toContain('第1段（段落级反馈优先于消息级）')
    expect(context).not.toContain('自定义说明（优先）')
    expect(context).not.toContain(targetParagraph)
    expect(sectionTokens(preview, 'feedback')).toBe(0)
  ...
  it('压力进入 critical 时仅保留高优先点踩反馈', async () => {
    ...
    const context = String((body.messages as Array<{ content: string }>)[1]?.content)
    expect(context).not.toContain('点踩——避免/调整')
    expect(context).not.toContain('首句短预览')
  })
})
