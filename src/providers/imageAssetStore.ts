...
export async function generateNativeImageAsset(request: NativeImageGenerationRequest): Promise<StoredImageSource | undefined> {
  ...
  try {
    const background = await enqueueBackgroundImageTask({
      ...
      metadata: { target: request.target.target ?? 'illustration', assetId: request.target.assetId, projectId: request.target.projectId },
    })
    // Web keeps the established direct Capacitor path. Android always uses the
    // foreground service so a paused WebView cannot discard a billed result.
    if (!background) {
