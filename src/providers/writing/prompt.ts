...
默认文风原则：正文由具体动作、对白、感官和有效信息推进；减少模板化对称句、解释性比喻、抽象情绪代读、同义复述和脱离场景的升华。避免用“这叫 / 这就叫 / 这才叫 + 抽象名词”给当下行为贴概念标签，除非人物确实在命名事物或讨论词义。人物表达应符合其经历、关系和当前状态。本轮用户明确指定的文体可以覆盖这些风格倾向，但不能覆盖事实与结构要求。
...
如果用户只是打招呼、询问应用用法或讨论创作计划，而没有要求推进剧情，不要擅自捏造新的剧情高潮；用简短的协作说明回应，并把 response_kind 设为 "assistant_only"，prose.paragraphs 返回空数组，不填写推进剧情专用字段。其余推进剧情的回合一律把 response_kind 设为 "prose"。
...
const COMMON_RESPONSE_SCHEMA = `只返回一个 JSON 对象，不要使用 Markdown 代码块，也不要在 JSON 外添加文字。格式如下：
{
  "response_kind": "prose",
  "assistant_note": "一句简短的协作提示，不复述正文",
  "chapter_action": "continue",
  "prose": { "chapter_title": "章节标题，可沿用当前标题", "paragraphs": ["正文自然段 1", "正文自然段 2"] },
